...
class FaceDiffuserModel(nn.Module):
    def __init__(self, text_encoder, image_encoder, vae, unet, args):
        ...
        self.postfuse_module = FaceDiffuserPostfuseModule(embed_dim)
    ...
    def from_pretrained(args):
        text_encoder = FaceDiffuserTextEncoder.from_pretrained(
        ...
        image_encoder = FaceDiffuserCLIPImageEncoder.from_pretrained(
            args.image_encoder_name_or_path,
        )

        return FaceDiffuserModel(text_encoder, image_encoder, vae, unet, args)
    ...
    def forward(self, batch, noise_scheduler):
        ...
        image_token_mask = batch["image_token_mask"] # b 77
        object_pixel_values = batch["object_pixel_values"] # b num_object c h w
        ...
        if self.object_localization:
            object_segmaps = batch["object_segmaps"] # b object_num h w
            image_token_idx = batch["image_token_idx"] # b object_num
            image_token_idx_mask = batch["image_token_idx_mask"] # b object_num
